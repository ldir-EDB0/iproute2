/*
 * m_conntrack.c		netfilter conntrack dscp<->conntrack mark action
 *
...
#include "utils.h"
#include "tc_util.h"
#include <linux/tc_act/tc_conntrack.h>

static const char * conntrack_modes[] = {
	"?invalid",
	"dscp",
};
...
explain(void)
{
	fprintf(stderr, "Usage: ... conntrack mask MASK statemask STATEMASK mode dscp [zone ZONE] [CONTROL] [index <INDEX>]\n");
	...
	if (argc) {
		if (matches(*argv, "mask") == 0) {
			...
			if (get_u32(&sel.mask, *argv, 0)) {
				fprintf(stderr, "conntrack: Illegal \"mask\"\n");
				return -1;
			}
			argc--;
			argv++;
	...
	if (argc) {
		if (matches(*argv, "statemask") == 0) {
			...
			if (get_u32(&sel.statemask, *argv, 0)) {
				fprintf(stderr, "conntrack: Illegal \"statemask\"\n");
				return -1;
			}
			argc--;
			argv++;
	...
	if (argc) {
		if (matches(*argv, "mode") == 0) {
			...
			if (matches(*argv, "dscp") == 0)
				sel.mode |= CONNTRACK_FLAG_SETDSCP;
			else {
				fprintf(stderr, "conntrack: Illegal \"mode\"\n");
				return -1;
			}
			argc--;
			argv++;
	...
	if (argc) {
		if (matches(*argv, "zone") == 0) {
			...
			if (get_u16(&sel.zone, *argv, 10)) {
				fprintf(stderr, "conntrack: Illegal \"zone\"\n");
				return -1;
			}
			argc--;
			argv++;
	...
	if (argc) {
		if (matches(*argv, "index") == 0) {
			...
			if (get_u32(&sel.index, *argv, 10)) {
				fprintf(stderr, "conntrack: Illegal \"index\"\n");
				return -1;
			}
			argc--;
			argv++;
...
static int print_conntrack(struct action_util *au, FILE *f, struct rtattr *arg)
{
	...
	if (tb[TCA_CONNTRACK_PARMS] == NULL) {
		print_string(PRINT_FP, NULL, "%s", "[NULL conntrack parameters]");
		return -1;
	}
	...
	print_string(PRINT_ANY, "mode", " mode %s", conntrack_modes[ci->mode & CONNTRACK_FLAG_SETDSCP]);
...
struct action_util conntrack_action_util = {
	.id = "conntrack",
